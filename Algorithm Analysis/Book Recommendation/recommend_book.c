...
void predicate_user(user nu_user, int nu_order, user u_users[], int k, int **k_max_list, char **book_names, int book_count);

// test function
int **test1(int k, user nu_users[], int nu_count, user u_users[], int u_count, int book_count);

void test2and3(user nu_users[], int nu_count, user u_users[], int **k_max_list, int k, char **book_names, int book_count);
...
int main() {
    ...
    printf("\t- This application reads NU and U users from a csv file.\n");
    printf("\t- Application recommends books to NU users based on resemblance of U users.\n");
    printf("\t- User can select similar user count for adjusting recommendation.\n");
...
float calculate_pearson(user u1, user u2, int book_count) {
    ...
    for (i = 0; i < book_count; i++) {
        ...
        if (u1.book_list[i] > 0 && u2.book_list[i] > 0) {
            average1 += (float)u1.book_list[i];
            average2 += (float)u2.book_list[i];
            common_book++;
        }
    }
    ...
    for (i = 0; i < book_count; i++) {
        ...
        if (u1.book_list[i] > 0 && u2.book_list[i] > 0) {
            ...
            denominator1 += (float)pow(((float) u1.book_list[i] - average1), 2);
            denominator2 += (float)pow(((float) u2.book_list[i] - average2), 2);
        }
    }
    // calculate result
    return numerator / (float)(sqrt((double)denominator1) * sqrt((double)denominator2));
}
