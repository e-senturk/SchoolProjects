...
parameters:
	hash_table = an array holding the hash table
	load_factor = holding the number of filled elements in the table
	select = character used when receiving input from the user
	file_name = file's name
	string = holding the word to search
*/
int main() {
    word *hash_table;
    int load_factor;
...
parameters:
	c = a character
	return = integer value of the alphabetical order of the character
*/
int order(char c) {
    if (c >= 'A' && c <= 'Z')
        return c - 'A';
...
parameters:
	c = a character
	return = lowercase version of the given character
*/
...
parameters:
	c1 = 1. character
	c2 = 2. character
	return = boolean value that indicates whether the characters are equal or not
*/
...
parameters:
	...
	s2 = 2. string
	i = iterator
	return = boolean value that indicates whether the strings are equal or not
*/
...
parameters:
	...
	value = key-word
	return = boolean value containing information on whether the word was found or not
*/
bool find_list(list *head, char *value) {
...
parameters:
	head = head node of the list
	new_head = new head node of the list
	value = key-word
...
parameters:
	...
	new_word = a struct containing the word and the document names
	return = new_word struct
*/
word create_word(char *string, char *document) {
    word new_word;
...
algorithm: text calculated from first index to to last with a for loop and calculated a sum value.
	...
	if the result is negative (since the probability of the sum exceeding the integer dimension), the result was multiplied by -1.

parameters:
	text = any text
	i = iterator
	sum = storing sum calculated with horner
	size = text size
	return = horner value
*/
int horner(char *text) {
...
parameters:
	string = any text
	i = h2 coefficient
	horn = integer number calculated by horner method
	h1 = h1 function result
	h2 = h2 function result
...
/*
the function that inserts the given string into the hash table
...
parameters:
	...
	step = number of steps
	index = calculated hash index
	ratio = variable that stores the fill rate of the table
*/
void insert_hash_table(char *string, char *document, word *hash_table, int *load_factor) {
    int step = 0;
...
/*
the function that finds the search word in the hash table and prints in which files
...
/*
The function that writes the hash table and load factor from the given database
...
parameters:
	...
	i = iterator
	database = pointer that stores the database file to be read
	load_str = string value of the load factor
	return = boolean value, which indicates whether the writing process was successful or failed
*/
bool write_database(word *hash_table, int load_factor) {
    int i;
    FILE *database;
...
/*
Function that reads hash table and load factor from database
...
parameters:
	...
	database = pointer that stores the database file to be read
	load_str = string value of the load factor
	temp,word = buffers that temporarily store input values
	string = stores read word
	return = hash table generated with reading
*/
word *read_database(int *load_factor) {
    int i;
    word *hash_table = (word *) malloc(sizeof(word) * TABLE_SIZE);
...
parameters:
	...
	hash_table = hash table to be created
	load_factor = element count of the table
	file = pointer that stores the file to be read
	string = read word
	return = boolean value indicating whether the file read operation was successful or failed
*/
bool read_document(char *file_name, word *hash_table, int *load_factor) {
    FILE *file;
    char string[100];
